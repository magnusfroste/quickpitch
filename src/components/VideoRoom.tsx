...
const VideoRoom = () => {
  ...
  const participantCount = users.length + 1;

  const { timeLeft, isExpired } = useRoomTimer(channelName, isHost, participantCount);
      ...
      <div className="h-full flex flex-col">
        <div className="flex justify-center items-center py-2 bg-black/10">
          <div className="text-lg font-semibold text-white bg-black/20 px-4 py-1 rounded-full">
            {participantCount < 2 ? "Waiting for participants..." : timeLeft}
          </div>
        </div>
